...
class ClaimDetails(models.Model):
    ...
    name = fields.Char(string='Name', required=True, copy=False,
                       readonly=True, index=True, default=lambda self: _('Nouveau'))
    name_2 = fields.Char(
    ...
    date_claimed = fields.Date(
        string='Date Appliqué', default=fields.Date.context_today)
        ...
        if not self.invoice_id:
            invoice_val = self.env['account.move'].sudo().create({
                ...
                'invoice_line_ids': [(0, 0, {
                    'name': 'Facture de Remboursement Assurance',
                    'quantity': 1,
